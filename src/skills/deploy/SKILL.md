...
- 所以所有改动必须**一次全推**，不能分开推好多次
- 柳柳确认后，把dev上所有改动一次性推到main
- 禁止中途多次推送
...
- `--no-bundle`不会自动开启find_additional_modules，必须显式写
...
- **推一次main = 触发一次Cloudflare部署，所以一次全推！**
- 改完每个步骤要自己检查一遍
- **改设定前必须先贴方案给柳柳确认**

## 最近使用记录（用完更新）
- 2026-08-21：柳柳提醒推main要一次全推（推一次=触发一次Cloudflare部署），已强化到铁律区
- 2026-08-21：今天推了好多次main被妹妹说了，以后记住一次搞定
