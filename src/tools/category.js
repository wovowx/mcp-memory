        ...
        if (!safeArgs.category) {
            ...
            if (matched.length === 0) {
                ...
                if (page.length === 0) {
                    ...
                    for (const item of page) { const title = getTitle(item.key); const time = String(formatTime(item.created_at) || '未知'); lines += '- ' + title + ': ' + item.value + '\n  💡 ' + time + '\n'; }
        ...
        if (!safeArgs.key || !safeArgs.newCategory) {
            ...
            if (!existing) {
                ...
                if (duplicate) {
                    ...
                    const data = { value: existing.value, created_at: existing.created_at, updated_at: new Date().toISOString() };
