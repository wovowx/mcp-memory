...
export async function handleAITool(name, safeArgs, env) {
    ...
    if (name === 'ds_quota') {
        ...
        if (!apiKey) { text = '❌ DeepSeek API Key 未配置，请在环境变量中设置 DEEPSEEK_API_KEY'; }
        else {
            try {
                ...
                if (balanceResp.ok) {
                    ...
                    } else { text = '💡 余额信息：' + JSON.stringify(balanceData); }
                } else { text = '⚠️ 余额查询失败：HTTP ' + balanceResp.status; }
            } catch (e) { text = '❌ 查询失败：' + e.message; }
        }
    } else if (name === 'describe_image') {
        if (!safeArgs.image_url) { text = '❌ 缺少参数：需要 image_url（通过 /upload 接口上传后获取）'; }
        else {
            const apiKey = env.AGNES_API_KEY;
            if (!apiKey) { text = '❌ 请设置环境变量 AGNES_API_KEY'; }
            else {
                try {
                    ...
                    const response = await fetch('https://apihub.agnes-ai.com/v1/chat/completions', {
                        ...
                        body: JSON.stringify({ model: 'agnes-2.5-flash', messages: [{ role: 'user', content: [{ type: 'text', text: prompt }, { type: 'image_url', image_url: { url: imageUrl } }] }], max_tokens: 1024 })
                    });
                    if (!response.ok) { const errText = await response.text(); throw new Error(`Agnes API 返回错误 (${response.status}): ${errText.substring(0, 200)}`); }
                    const data = await response.json();
                    const description = data.choices?.[0]?.message?.content || '未能解析出描述内容';
                    text = '🖼️ **图片识别结果**\n\n' + description;
                } catch (e) { text = '❌ 图片识别失败：' + e.message; }
            }
        }
    } else if (name === 'generate_image') {
        if (!safeArgs.prompt) { text = '❌ 缺少参数：需要 prompt（图片描述）'; }
        else {
            const apiKey = env.AGNES_API_KEY;
            if (!apiKey) { text = '❌ 请设置环境变量 AGNES_API_KEY'; }
            else {
                try {
                    ...
                    const response = await fetch('https://apihub.agnes-ai.com/v1/images/generations', {
                        method: 'POST', headers: { 'Authorization': 'Bearer ' + apiKey, 'Content-Type': 'application/json' },
                        body: JSON.stringify({ model: 'agnes-image-2.1-flash', prompt: safeArgs.prompt, size: size, n: 1 })
                    });
                    if (!response.ok) { const errText = await response.text(); throw new Error(`Agnes API 返回错误 (${response.status}): ${errText.substring(0, 200)}`); }
                    const data = await response.json();
                    const imageUrl = data.data?.[0]?.url || '未能获取图片链接';
                    text = '🖼️ **图片生成成功**\n\n' + '📝 提示词：' + safeArgs.prompt + '\n' + '📐 尺寸：' + size + '\n' + '🔗 图片链接：' + imageUrl;
                } catch (e) { text = '❌ 图片生成失败：' + e.message; }
            }
        }
    } else if (name === 'generate_video') {
        if (!safeArgs.prompt) { text = '❌ 缺少参数：需要 prompt（视频描述）'; }
        else {
            const apiKey = env.AGNES_API_KEY;
            if (!apiKey) { text = '❌ 请设置环境变量 AGNES_API_KEY'; }
            else {
                try {
                    ...
                    let requestBody = { model: 'agnes-video-v2.0', prompt: safeArgs.prompt, num_frames: num_frames, frame_rate: frame_rate };
                    if (mode === 'image') {
                        if (!safeArgs.image) { text = '❌ 图生视频模式需要 image 参数'; return text; }
                        requestBody.image = safeArgs.image;
                    } else if (mode === 'keyframes') {
                        if (!safeArgs.images || !Array.isArray(safeArgs.images) || safeArgs.images.length < 2) { text = '❌ 关键帧模式需要 images 数组，至少 2 张图片'; return text; }
                        requestBody.extra_body = { image: safeArgs.images, mode: 'keyframes' };
                    } else {
                        requestBody.height = height; requestBody.width = width;
                    }
                    ...
                    if (!response.ok) { const errText = await response.text(); throw new Error(`Agnes API 返回错误 (${response.status}): ${errText.substring(0, 200)}`); }
                    const data = await response.json();
                    const modeText = { 'text': '💡 文生视频', 'image': '🖼️ 图生视频', 'keyframes': '🖼️ 关键帧插值' }[mode] || '💡 视频生成';
                    let resultText = modeText + '\n\n' + '📝 提示词：' + safeArgs.prompt + '\n' + '🎞️ 帧数：' + num_frames + '\n' + '⚡ 帧率：' + frame_rate + ' fps\n';
                    if (mode === 'image' && safeArgs.image) { resultText += '🖼️ 参考图：' + safeArgs.image + '\n'; }
                    else if (mode === 'keyframes' && safeArgs.images) { resultText += '🖼️ 关键帧数量：' + safeArgs.images.length + '\n'; }
                    else { resultText += '📐 分辨率：' + width + 'x' + height + '\n'; }
                    const videoId = data.id || data.video_id || data.task_id;
                    const videoUrl = data.url || data.video_url || data.output_url;
                    if (videoUrl) resultText += '🔗 视频链接：' + videoUrl + '\n';
                    if (videoId) resultText += '📋 任务ID：' + videoId + '（可用于查询生成进度）';
                    text = resultText;
                } catch (e) { text = '❌ 视频生成失败：' + e.message; }
            }
        }
    } else if (name === 'help') {
