...
async function putMemory(env, key, data) {
    ...
    if (existing) {
        ...
        await fetch(`${supabaseUrl}/rest/v1/memories`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${supabaseKey}`, 'apikey': supabaseKey, 'Content-Type': 'application/json' },
            body: JSON.stringify({ key: key, value: data.value, category: category, title: title, created_at: data.created_at || now(), updated_at: data.updated_at || now() })
        });
    }
}
...
export async function handleMemoryTool(name, safeArgs, env) {
    ...
    if (name === 'remember') {
        if (!safeArgs.key || !safeArgs.value) { text = '❌ 缺少参数：需要 key 和 value'; }
            ...
            if (existing) { text = '⚠️ 该记忆已存在。如需修改内容请使用 update，如需修改分类请使用 move_category'; }
            else {
        ...
        if (!safeArgs.key || !safeArgs.value) { text = '❌ 缺少参数：需要 key 和 value'; }
            ...
            const existing = await getMemoryByKey(env, safeArgs.key);
            if (!existing) { text = '❌ 没有找到 "' + safeArgs.key + '"，无法更新。如需新建请使用 remember'; }
            else {
                const data = { value: safeArgs.value, updated_at: now() };
        ...
        if (!safeArgs.old_key || !safeArgs.new_key) { text = '❌ 缺少参数：需要 old_key 和 new_key'; }
        else if (safeArgs.old_key === safeArgs.new_key) { text = '⚠️ 新旧 key 相同，无需修改'; }
        else {
            ...
            if (!existing) { text = '❌ 没有找到 "' + safeArgs.old_key + '"'; }
                ...
                if (duplicate) { text = '⚠️ 目标 key "' + safeArgs.new_key + '" 已存在，请先处理冲突'; }
                else {
                    await deleteMemory(env, safeArgs.old_key);
                    const data = { value: existing.value, created_at: existing.created_at, updated_at: now() };
        ...
        if (all.length === 0) { text = '💡 暂无记忆，还没有分类'; }
        else {
            const categoryMap = {};
            ...
            for (const [cat, count] of sorted) lines += '- ' + cat + '（' + count + '条）\n';
            text = lines;
        }
        ...
        if (!safeArgs.category) { text = '❌ 缺少参数：需要 category'; }
            ...
            if (matched.length === 0) { text = '💡 分类 "' + safeArgs.category + '" 下暂无记忆'; }
            else {
                matched.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
                let startIndex = 0;
                if (cursor) { const idx = matched.findIndex(item => item.id === cursor); if (idx !== -1) startIndex = idx + 1; }
                ...
                if (page.length === 0) { text = '💡 没有更多记忆了'; }
                else {
                    let lines = '💡 分类 "' + safeArgs.category + '" 的记忆（共 ' + matched.length + ' 条，显示 ' + page.length + ' 条）：\n';
                    for (const item of page) { const title = getTitle(item.key); lines += '- ' + title + ': ' + item.value + '\n  💡 ' + formatTime(item.created_at) + '\n'; }
        ...
        if (!safeArgs.key || !safeArgs.newCategory) { text = '❌ 缺少参数：需要 key 和 newCategory'; }
            ...
            if (!existing) { text = '❌ 没有找到 "' + safeArgs.key + '"'; }
                ...
                if (duplicate) { text = '⚠️ 目标 key "' + newKey + '" 已存在，请先处理冲突'; }
                else {
                    await deleteMemory(env, safeArgs.key);
                    const data = { value: existing.value, created_at: existing.created_at, updated_at: now() };
        ...
        if (!safeArgs.keyword) { text = '❌ 请输入搜索关键词'; }
            ...
            const page = matched.slice(startIndex, startIndex + limit);
            const nextCursor = matched.length > startIndex + limit ? page[page.length - 1]?.id : null;
            if (page.length === 0) { text = matched.length === 0 ? '💡 没有找到匹配的记忆' : '💡 没有更多匹配结果了'; }
            else {
        ...
        const all = await getAllMemories(env);
        if (all.length === 0) { text = '💡 暂无记忆，先存一条吧～'; }
        else {
            all.sort((a, b) => { const ta = new Date(a.created_at); const tb = new Date(b.created_at); return order === 'asc' ? ta - tb : tb - ta; });
            let startIndex = 0;
            if (cursor) { const idx = all.findIndex(item => item.id === cursor); if (idx !== -1) startIndex = idx + 1; }
            ...
            if (page.length === 0) { text = '💡 没有更多记忆了'; }
                ...
                for (const item of page) { const category = getCategory(item.key); const title = getTitle(item.key); lines += '- [' + category + '] ' + title + ': ' + item.value + '\n  💡 ' + formatTime(item.created_at) + '\n'; }
                if (nextCursor) lines += '\n💡 next_cursor: ' + nextCursor + '（继续翻页请传此值）';
                text = lines;
            }
        }
        ...
        if (all.length === 0) { text = '💡 暂无记忆，先存一条吧～'; }
            ...
            if (category) {
                ...
                if (candidates.length === 0) text = '❌ 分类 "' + category + '" 下暂无记忆';
            }
        ...
        if (!safeArgs.start_date || !safeArgs.end_date) { text = '❌ 缺少参数：需要 start_date 和 end_date'; }
        else if (!isValidDate(safeArgs.start_date) || !isValidDate(safeArgs.end_date)) { text = '❌ 日期格式无效，请使用 YYYY-MM-DD 格式'; }
        else {
            const { limit, cursor } = parsePagination(safeArgs);
            ...
            const all = await getAllMemories(env);
            const matched = [];
            for (const item of all) { const created = new Date(item.created_at); if (created >= start && created <= end) matched.push(item); }
            matched.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
            let startIndex = 0;
            if (cursor) { const idx = matched.findIndex(item => item.id === cursor); if (idx !== -1) startIndex = idx + 1; }
            const page = matched.slice(startIndex, startIndex + limit);
            const nextCursor = matched.length > startIndex + limit ? page[page.length - 1]?.id : null;
            if (page.length === 0) { text = matched.length === 0 ? '💡 该时间段内没有记忆' : '💡 没有更多结果了'; }
            else {
        ...
        if (all.length === 0) { text = '💡 统计：\n总条数：0\n暂无记忆，先存一条吧～'; }
            ...
            for (const item of all) {
                ...
                if (new Date(item.created_at) >= oneWeekAgo) weekCount++;
            }
        ...
        if (suggestions.length === 0) { text = prefix ? '💡 没有匹配的联想词' : '💡 暂无搜索历史，多搜几次就会有联想了～'; }
        else {
            let lines = '💡 联想词：\n';
            for (const word of suggestions) lines += '- ' + word + '\n';
            text = lines;
        }
    }
    return text;
