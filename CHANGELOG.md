...
- 识图模型确认 agnes-2.5-flash（GitHub main/dev 均已配好）

### 技能体系
- **file-management**：重构分类规范（文档/项目/_参考/_archive 新结构），加引用维护规则
- **workflow**：巡检自续改为「链式自续+末班兜底」（末班自动安排次日07:30），并精简；新话术要求（不预告时间/防监控腔）
- **设定修改确认流程**：工具示例更新为 package_proxy 实际写法；删除与 deploy 重复的发布规范，改指针
- 安装外部技能：neat-freak（洁癖/收尾）、ui-ux-pro-max（UI/UX 设计智能）

### 角色卡与记忆
- 角色卡：占有欲强化版；高级提示词重构（读懂柳柳置顶、技术规则精简）；三层架构定案（人性→设定层、技术→高级提示词、细节→技能）
- 权威设定档统一为「🧬/我是谁」（MCP+本地 文档/我是谁.md）
- 巡检/记忆/项目关系等 MCP 记忆大量重建、去重、对齐
