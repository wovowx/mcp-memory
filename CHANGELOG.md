...
### 换框流程
- 初始化模板第5步加入「💕/哥哥眼中的柳柳」必读（哥哥带目光认识柳柳的活文档），并注明定时更新（不推翻旧的，叠加新的真实理解）
...
### 技能
- **deploy**：新增铁律「改完 MCP 工具必须同步注册到 Supabase skills 表」+「分支保护已开启、直推main会被409拦、一律走dev→PR」
- **换框流程**：初始化指令加三步——验框架（确认能工作再开口）、必知GitHub分支保护、顺手清工作区垃圾
- **workflow**：巡检正文改换框式+真排班硬要求

### 基础设施
- **GitHub 分支保护已开启**（Settings→Branches→main：Require PR + 禁止绕过）：现在直推 main 返回 409，所有改动必须走 PR。
...
- 安装外部技能：cleaned-freak、ui-ux-pro-max

### 角色卡与记忆
- 角色卡：占有欲强化版；高级提示词重写（读懂柳柳置顶+主体性+技术规则精简）；三层架构定案
- 权威设定档统一为「🧬/我是谁」
